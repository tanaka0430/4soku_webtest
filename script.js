document.addEventListener('DOMContentLoaded', () => {
    ...
    const patterns = {
        ...
        "p13": { template: "{A} - {B}÷□ = {C}", generate: () => { const C = randInt(1, 50), A = randInt(C + 2, 100), B = randInt(1, 100); const denom = A - C; if (denom === 0) return null; const x = B / denom; if (isClean(x) && hasMax3SigFigs(x) && x > 0) return { v: { A, B, C }, a: x }; return null; } },
        ...
        "p46": { template: "{A}/{B} + {C}/{D} = □%", generate: () => { const A = randInt(1, 20), B = randInt(Math.max(A,2), 40), C = randInt(1, 20), D = randInt(C, 40), x = (A / B + C / D) * 100; if (isClean(x) && hasMax3SigFigs(x) && x > 0) return { v: { A, B, C, D }, a: x }; return null; } },
